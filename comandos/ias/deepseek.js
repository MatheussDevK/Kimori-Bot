...
module.exports = {
name: 'deepseek',
aliases: ['deepseekv4', 'deepseekpro'],
category: 'ias',
description: 'Conversa com o modelo DeepSeek V4 Pro.',
async execute(ctx) {
...
await reagir(from, "💾");
reply(`💭 *Pensando... (DeepSeek V4 Pro)*`);
try {
const url = `${API_KIMORI_URL}/api/ai/deepseek/pro?q=${encodeURIComponent(q.trim())}&apikey=${APIKEY_KIMORI}`;
const response = await fetch(url);
const data = await response.json();
if (!data.success) return reply(`❌ Erro: ${data.error || 'Falha ao processar'}`);
let msg = `💾 *DEEPSEEK V4 PRO*\n\n`;
