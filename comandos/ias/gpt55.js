...
module.exports = {
name: 'gpt55',
aliases: ['gpt5.5', 'chatgpt55'],
category: 'ias',
description: 'Conversa com o modelo OpenAI GPT-5.5.',
async execute(ctx) {
...
await reagir(from, "💬");
reply(`💭 *Pensando... (GPT-5.5)*`);
try {
const url = `${API_KIMORI_URL}/api/ai/openai/gpt55?q=${encodeURIComponent(q.trim())}&apikey=${APIKEY_KIMORI}`;
const response = await fetch(url);
const data = await response.json();
if (!data.success) return reply(`❌ Erro: ${data.error || 'Falha ao processar'}`);
let msg = `💬 *OPENAI GPT-5.5*\n\n`;
