...
module.exports = {
name: 'gemini3',
aliases: ['gemini3.0', 'gemini3pro', 'gemini2.0', 'geminiai', 'gemini'],
category: 'ias',
description: 'Conversa com o modelo Google Gemini 3 Pro.',
async execute(ctx) {
...
await reagir(from, "🔵");
reply(`💭 *Pensando... (Gemini 3 Pro)*`);
try {
const url = `${API_KIMORI_URL}/api/ai/gemini/3pro?q=${encodeURIComponent(q.trim())}&apikey=${APIKEY_KIMORI}`;
const response = await fetch(url);
const data = await response.json();
if (!data.success) return reply(`❌ Erro: ${data.error || 'Falha ao processar'}`);
let msg = `🔵 *GOOGLE GEMINI 3 PRO*\n\n`;
