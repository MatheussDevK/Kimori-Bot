...
name: 'ytaudio1',
category: 'downloads',
description: 'Baixa o áudio de um vídeo do YouTube (versão 1).',
...
if (
!q?.trim() ||
(!q.includes('youtube.com') && !q.includes('youtu.be'))
) {
return reply(
`🎵『${emojii}❌』Ops, rum você usou da forma errada, use assim: ${prefix + command} https://youtu.be/SmWQg6G9Pxg`
);
}

try {
const url =
`${API_KIMORI_URL}/api/dl/ytaudio1?` +
`url=${encodeURIComponent(q.trim())}` +
`&apikey=${APIKEY_KIMORI}`;
