const { rnksafada } = require('../../config-bot/logos/links_img.json');
module.exports = {
name: 'ranksafada',
aliases: ['ranksafadas'],
category: 'brincadeiras',
description: 'Gera um rank aleatório de safadas do grupo.',
async execute(ctx) {
const { isGroup, isModobn, mess, prefix, somembros, kiimorizinha, from, ChannelContextNewsLetter, selo, reply } = ctx;
if(!isGroup) return reply(mess.onlyGroup())
if(!isModobn) return reply(mess.onlyGroupFun(prefix))
let mentionsSafada = []
let rankSafada = `🔥 RANK DAS 5 MAIS SAFADINHAS DO GRUPO\n\n`
for(let i=0;i<5;i++){
const membro = somembros[Math.floor(Math.random()*somembros.length)]
mentionsSafada.push(membro)
rankSafada += `• ${i+1}° ${Math.floor(Math.random()*100)}% - @${membro.split('@')[0]}\n\n`
}
await kiimorizinha.sendMessage(from, {image: {url: rnksafada}, caption: rankSafada, contextInfo:{...ChannelContextNewsLetter, mentionedJid: mentionsSafada}}, {quoted: selo})
}
};
