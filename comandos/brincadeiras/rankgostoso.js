const { rnkgostoso } = require('../../config-bot/logos/links_img.json');
module.exports = {
name: 'rankgostoso',
aliases: ['rankgostosos'],
category: 'brincadeiras',
description: 'Gera um rank aleatório de gostosos do grupo.',
async execute(ctx) {
const { isGroup, isModobn, mess, prefix, somembros, kiimorizinha, from, ChannelContextNewsLetter, selo, reply } = ctx;
if(!isGroup) return reply(mess.onlyGroup())
if(!isModobn) return reply(mess.onlyGroupFun(prefix))
let mentionsGostoso = []
let rankGostoso = `🔥 RANK DOS 5 MAIS GOSTOSOS DO GRUPO\n\n`
for(let i=0;i<5;i++){
const membro = somembros[Math.floor(Math.random()*somembros.length)]
mentionsGostoso.push(membro)
rankGostoso += `• ${i+1}° ${Math.floor(Math.random()*100)}% - @${membro.split('@')[0]}\n\n`
}
await kiimorizinha.sendMessage(from, {image: {url: rnkgostoso}, caption: rankGostoso, contextInfo:{...ChannelContextNewsLetter, mentionedJid: mentionsGostoso}}, {quoted: selo})
}
};
