...
const { 
waWebSocketUrl, connectTimeoutMs, logger, keepAliveIntervalMs, 
browser, auth: authState, printQRInTerminal, defaultQueryTimeoutMs, 
...
logger.warn({}, '⚠️ The printQRInTerminal option has been deprecated...');
}

const syncDisabled = PROCESSABLE_HISTORY_TYPES
.map(syncType => config.shouldSyncHistoryMessage({ syncType }))
.filter(x => x === false).length === PROCESSABLE_HISTORY_TYPES.length;
...
sendPromise.call(ws, bytes).then(resolve).catch(reject);
});
};
...
if (!node.attrs.id) {
node.attrs.id = `${uqTagId}${state.epoch++}`;
}
const msgId = node.attrs.id;
const result = await promiseTimeout(timeoutMs, async (resolve, reject) => {
const msgPromise = waitForMessage(msgId, timeoutMs).catch(reject);
sendNode(node).then(() => msgPromise.then(resolve)).catch(reject);
});
...
const generateMessageTag = () => `${uqTagId}${state.epoch++}`;
...
content: usyncQuery.protocols.map(a => a.getUserElement(user)).filter(Boolean)
}));
const iq = {
tag: 'iq',
...
if (state.uploadPreKeysPromise) {
logger.debug('Pre-key upload already in progress, waiting for completion');
await state.uploadPreKeysPromise;
return;
}
...
state.uploadPreKeysPromise = Promise.race([
uploadLogic(0),
...
state.uploadPreKeysPromise = null;
}
};
...
state.keepAliveReq = setInterval(() => {
if (!state.lastDateRecv) state.lastDateRecv = new Date();
const diff = Date.now() - state.lastDateRecv.getTime();
...
clearInterval(state.keepAliveReq);
...
for (const handler of state.socketEndHandlers) {
try { await handler(error); } catch (err) {
...
state.socketEndHandlers.push(handler);
};
...
state.qrTimer = setTimeout(genPairQR, qrMs);
...
try { await digestKeyBundle(); } catch (e) {
...
void end(new Boom('Connection Failure', { statusCode: +(node.attrs.reason || 500), data: node.attrs }));
});
...
ws.on('CB:ib,,offline_preview', async () => {
